...
public class Phoneketmon {
    ...
    public int solution(int[] nums) {
        ...
        // 중복된 값들을 제거
        List<Integer> dupList = numList.stream().distinct().collect(Collectors.toList());

        // 중복 제거된 사이즈와 선택가능한 갯수중 작은것을 리턴
        return dupList.size() < selectCnt ? dupList.size() : selectCnt;
    }

    public static void main(String[] args) {
        System.out.println( new Phoneketmon().solution(new int[]{3,1,2,3}) ); // answer: 2
        System.out.println( new Phoneketmon().solution(new int[]{3,3,3,2,2,4}) ); // answer: 3
        System.out.println( new Phoneketmon().solution(new int[]{3,3,3,2,2,2}) ); // answer: 2
    }
}
